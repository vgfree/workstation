...
local sql_fmt = {
    getAccountIDMobileInfo = "SELECT accountID, mobile FROM userLoginInfo WHERE userStatus=1 AND mobile='%s'",
    getImeiAccountIDInfo = "SELECT imei FROM userLoginInfo WHERE userStatus=1 AND mobile='%s'",
}
...
local function getAccountIDMobile(mobile_tab)
    ...
    for i = 1, #mobile_tab do
        ...
        if not ok then
            only.log("E", "get AccountID Mobile mysql is error! %s", sql)
        end
...
local function getImeiAccountID(res)
    ...
    for i = 1, #res do
        ...
        if ok and #result>0 and #result[i]['imei']==15 then
...
local function get_online_user(res)
    ...
    for i = 1, #res do
        ...
        if ok and ret and (cur_time-tonumber(ret) < cfg_time) then
