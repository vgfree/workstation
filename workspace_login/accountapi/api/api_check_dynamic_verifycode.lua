...
local sql_fmt = {
    ...
    sql_check_mobile = " SELECT accountID, nickname FROM userLoginInfo  WHERE mobile = '%s'",
    sql_check_userDynamicVerifycode = " SELECT updateTime , content FROM userDynamicVerifycode WHERE mobile = %s  and  validity = 1 ",
    sql_check_psw = "SELECT daokePassword FROM userLoginInfo  WHERE accountID = '%s' ",
    sql_update_pwd_userlist =  " UPDATE userLoginInfo SET daokePassword='%s', updateTime = %d WHERE accountID='%s' ",
...
local function check_parameter(res)
    ...
    if res['nickname'] and #res['nickname'] ~= 0 then
        if string.find( res['nickname'], "'" ) then
            only.log("E","nickname is error")
...
local function check_accountid_by_mobile( account_id , mobile, verifycode, password )
    ...
    if tostring(pwd_cod) == tostring(pwd) then
        if password and password ~= "" then
            ...
            local ok = mysql_api.cmd(userlist_dbname , 'update', sql_ret)
            if not ok then
...
local function post_add_custom_account(mobile, password,appKey, nickname)      
    local  tab = {
        appKey = appKey,
        mobile =  mobile,
        ...
        gender = 1 ,
        accountType = 2 ,
        nickname = nickname or string.sub(mobile,-4),
    }
...
local function get_oauth_verifycode(mobile, verifycode , appKey, password, nickname )
    ...
    if password and password ~= "" then
        ...
        account_id = post_add_custom_account( mobile, password , appKey, nickname)
    else
        ...
        account_id = post_add_custom_account( mobile, verifycode , appKey, nickname)
    end
...
local function handle()
    ...
    local ok, modifycode = redis_api.cmd('private', 'get', tmp_key )
    if not ok then
        only.log('E', string.format('redis failed!,value = %s', value))
        gosay.go_false(url_info, msg['MSG_DO_REDIS_FAILED'])
    end

    local account_id , nickname
    if not modifycode then
        only.log('D', "new custom login")
        ...
        if #result == 1 then
            ...
            account_id = get_oauth_verifycode (mobile, verifycode, appKey, password, nickname)
        end
        ...
        only.log('D', "old custom login")
        if tonumber(modifycode) == tonumber(verifycode) then
