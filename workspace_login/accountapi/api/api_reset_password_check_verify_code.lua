...
local G = {
    ...
    --检查用户输入的手机号
    sel_check_mobile = "select accountID from userLoginInfo where userStatus=1 and mobile='%s'",

    --校验accountID
    sel_check_account = " select 1 from userLoginInfo where accountID='%s' limit 1",
    ...
    --重置用户密码
    update_password = " update userLoginInfo set daokePassword='%s', updateTime=%d where accountID='%s'",
...
local function reset_password( args, accountID)
    -- local sql_tab = {}
    local cur_time = os.time()
    ...
    local ok_status, ret = mysql_api.cmd(user_dbname,'UPDATE',sql_str)
    if not ok_status then
