...
local sql_fmt = {
    ...
    sql_check_imei = "SELECT 1 FROM userLoginInfo WHERE imei= %s ",
...
local user_ident_dbname = "app_ident___ident"
...
local function handle()
	...
	local shortimei = string.sub(args['IMEI'],1,14)
	local nCheck = string.sub(args['IMEI'],15,15)
        sql = string.format(sql_fmt.sql_get_imei_info,shortimei,nCheck)
        ok, result = mysql_api.cmd(user_ident_dbname, 'select', sql)
        if not ok or not result then
