...
function after_set_nickname(accountid, nickname, status )
	...
	local jtxt2voice = link["OWN_DIED"]["http"]["jtxt2voice"]
	...
	-- local ok,file_url = app_utils.txt_2_voice(jtxt2voice,appKey,secret,tmp_nickname)
	...
	local ok,file_url = appfun.jtxt2voice(jtxt2voice,tab)
	if ok and file_url then
		redis_api.cmd('private', 'set', accountid .. ':nicknameURL',file_url)
		file_url = string.gsub(file_url,"%%","")
		only.log('D',string.format("accountid:%s   nickname: %s  url:%s  " , accountid, tmp_nickname, file_url ))
		return true
	else
		redis_api.cmd('private', 'del', accountid .. ':nicknameURL')
		only.log('E',string.format("nickname:%s to amr file_url failed!", string.gsub(tmp_nickname,"%%","")))
		return false
	end
end
