...
local sql_fmt = {
    ...
    sql_mobile_count = "SELECT count(1) as cnt FROM userLoginInfo WHERE userStatus=1 AND mobile='%s' ",
}

local usercenter_dbname = 'app_usercenter___usercenter'
local daokeme_dbname = 'app_daokeme___daokeme'
...
--]]
    ...
    only.log('D', sql)
    local ok, res = mysql_api.cmd(daokeme_dbname, 'insert', sql)
    if not ok then
...
local function get_mobile_count(mobile)
    ...
    local sql = string.format(sql_fmt.sql_mobile_count, mobile)
    only.log('D',sql)
...
local function handle()
    ...
    local ok, secret = redis_api.cmd("public", "hget", res["appKey"] .. ':appKeyInfo', 'secret')

    local code = get_code_info(mobile)
