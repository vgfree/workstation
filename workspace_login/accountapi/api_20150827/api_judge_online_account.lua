...
local sql_fmt = {
    sel_imei  = "SELECT imei FROM userList WHERE accountID = '%s'",
...
local function get_user_status(a_id)
    ...
    local ok, result = mysql_pool_api.cmd( coreIdentif_dbname, 'select', res_sql)
    if not ok then
        only.log('E', res_sql)
        gosay.go_false(url_info, msg["MSG_DO_MYSQL_FAILED"])
    end
    ...
    local ok, ret = redis_pool_api.cmd('private', 'get', a_id .. ':heartbeatTimestamp')
