...
local sql_fmt = {
    get_account_id = "SELECT accountID, id FROM loginType WHERE userStatus IN (1, 3) AND account='%s' AND loginType=%d",
    insert_record = "INSERT INTO loginType SET accountID='%s', loginType=%d, account='%s',token='%s', refreshToken='%s', accessToken='%s', accessTokenExpiration=%d",
    update_record = "UPDATE loginType SET token='%s', refreshToken='%s', accessToken='%s', accessTokenExpiration=%d WHERE id=%d",
}
...
local function handle()
    ...
    if #result == 0 then
        ...
        only.log('D', sql)
        ok, ret_req = mysql_pool_api.cmd('app_usercenter___usercenter', 'insert', sql)
        ...
        only.log('D', sql)
        ok, ret_req = mysql_pool_api.cmd('app_usercenter___usercenter', 'update', sql)
