...
function touch_media_redis(args_tab)
    ...
    only.log('D', string.format('zadd %s %s %s', key, args_tab['level'] .. js_tab['time'][1],  js_tab['bizid']))
    		...
    		if args_tab['receiverAccountID'] == "egQH09ks9a" or args_tab['receiverAccountID'] == "HRlYQhFvpW"  then 
  			only.log('E', string.format('lipengwei weibofun zadd %s %s %s', key, args_tab['level'] .. js_tab['time'][1],  js_tab['bizid']))
		end

    local ok, ret = redis_pool_api.cmd('weibo', 'zadd', key, args_tab['level'].. js_tab['time'][1],  js_tab['bizid'])
    if not ok then
        only.log('E', string.format('error zadd %s %s %s', key, args_tab['level'] .. js_tab['time'][1],  js_tab['bizid']))
        return false
    end
...
function group_save_message_to_msgredis(args)
    ...
    --KEY:sendmessage:time value:senderAccountID:appkey
    ...
    if args['groupID'] then
  	...
  	-- KEY:[accountid:appkey:groupID:time]:sendmessage value: time|URL
...
function personal_save_message_to_msgredis(args)
    ...
    --only.log('W',"cityCode:%s,senderType :%s commentID :%s",city_code,args['senderType'] , commentID)
