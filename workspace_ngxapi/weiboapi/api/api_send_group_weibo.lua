...
local function handle()
	...
	body_args['geometryType']		 = tonumber(body_args['geometryType'])
	...
	body_args['isOnline']			 = tonumber(body_args['isOnline'])
	body_args['isChannel']			 = tonumber(body_args['isChannel'])
	...
	if not ok then
	only.log('S',"json encode is error")
	gosay.go_false(url_info, msg['MSG_ERROR_REQ_ARG'], 'json_encode')
	end

	only.log('D',"json encode string: %s",string)

	local ok,bizid = redis_pool_api.cmd('weiboNew','SET','group', string)


	if not ok then
	only.log('S',"redis send string: %s",string)
