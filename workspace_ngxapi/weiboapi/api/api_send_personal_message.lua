...
local function check_parameters(tab)
    ...
    if tab['POIType']  and  #tab['POIType'] ~= 7 then
...
local function handle()
    ...
    if not tab['bizid'] then
        tab['bizid'] = weibo_fun.create_bizid('a4')
    end

    local ok,shutup = redis_pool_api.cmd('private','get',string.format("%s:shutup",tab['receiverAccountID']))
	...
	local ok,imei = true ,tab['receiverAccountID']
	
	if imei and #imei == 10  then
	
		ok, imei = redis_pool_api.cmd('private','get',imei ..':IMEI')
		if not ok then
			gosay.go_false(url_info,msg['MSG_DO_REDIS_FAILED'])
		end
	end

	local personal_table ={
		bizid = tab['bizid'],
...
local result_tab ={}
    ...
    if type(tab['multimediaURL']) == 'table' then
		for k,v in ipairs (tab['multimediaURL']) do
			...
			only.log('E', " %s get dfsurl %s " , tab['receiverAccountID'] ,  v)
			if dfs_data_isok == false or not dfs_data_binary then
				only.log('E', " %s get dfsurl %s data failed---->----return 555 " , tab['receiverAccountID'] , v )
				gosay.go_false(url_info, msg["MSG_ERROR_REQ_ARG"], "multimediaURL")
			end
		...
		---- 二进制文件小于300字节, 大于20K,不下发给终端 2014-11-26 
			if #dfs_data_binary < weibo_fun.FILE_MIN_SIZE or #dfs_data_binary >= weibo_fun.MAX_NEWSTATUS_FILE_LENGTH then
				only.log('W', " %s get dfsurl %s data succed but file length :%s [ERROR]" , G.cur_accountid , v ,#dfs_data_binary )
				gosay.go_false(url_info, msg["MSG_ERROR_REQ_ARG"], "multimediaURL")
			end
		...
		if  tab['multimediaURL'] ~= '' then
			--http://g1.tweet.daoke.me/group1/M00/00/00/rBALhFNNLg6ABKqHAAAotg-PQhY599.amr
			dfs_data_isok,urltype,dfs_data_binary = get_binary_from_dfs(tab['multimediaURL'])
			---- 1 true --->---succed   false  -->---failed
			---- 2 binary is file data binary
		end
		only.log('E', " %s get dfsurl %s " , tab['receiverAccountID'] ,  tab['multimediaURL'])
		if dfs_data_isok == false or not dfs_data_binary then
			only.log('E', " %s get dfsurl %s data failed---->----return 555 " , tab['receiverAccountID'] , tab['multimediaURL'] )
			gosay.go_false(url_info, msg["MSG_ERROR_REQ_ARG"], "multimediaURL")
		end

		---- 二进制文件小于300字节, 大于20K,不下发给终端 2014-11-26 
		if #dfs_data_binary < weibo_fun.FILE_MIN_SIZE or #dfs_data_binary >= weibo_fun.MAX_NEWSTATUS_FILE_LENGTH then
			only.log('W', " %s get dfsurl %s data succed but file length :%s [ERROR]" , tab['receiverAccountID'] , tab['multimediaURL'] ,#dfs_data_binary )
			gosay.go_false(url_info, msg["MSG_ERROR_REQ_ARG"], "multimediaURL")
		end
		result_tab[3] = dfs_data_type ..'\r\n'..dfs_data_binary
end
	
	only.log('D',"dfs_data_binary is %s",result_tab[3])
	personal_table['content']['mmfiletype'] = dfs_data_type
        personal_table['content']['mmfilelength'] = string.len(dfs_data_binary)

	local ok,second_frame = utils.json_encode(personal_table)
	...
	result_tab[1] = '0'..tostring(imei)
	result_tab[2] = 'json\r\n'..second_frame
	
	only.log('D',result_tab[1])
	only.log('D',result_tab[2])

	local ok = zmq_api.cmd("personal", "send_table", result_tab)
	if not ok  then
