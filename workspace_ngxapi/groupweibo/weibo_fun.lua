...
function touch_media_redis(args_tab)
	...
	--fix houyaqian 2015-09-29
	--redis memory warning
	--"set" is changed to "setex"
	--add a expire time
...
------统计变量修改时间2014-10-16---------
----统计所有appKey发送微博的总记录数
-- 在测试页面上点一次提交这里的totalNum值就+1，也即是所发送的总的记录数
function incrby_appkey(appKey, cur_month, cur_day)
	...
	-- 如 HINCRBY 20160307:weiboAppKeyTotalInfo 2064302565:totalNum 
...
function groupid_update_timestamp( groupid )
	-- hmset key field value [field1 value1] 将field-value对设置到哈希表key中
...
function message_info_key(args,media_url,sender_lon,sender_lat,receiver_lon,receiver_lat,city_code,city_name)
	local value = string.format("%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s",
	args['appKey'],
	args['sourceID'] or '',
	args['sourceType'],
	args['fileID'] or '',
	media_url,
	args['senderAccountID'] or '',
	sender_lon, 
	sender_lat,
	args['senderDirection'] or 0,
	args['senderSpeed'] or 0,
	args['senderAltitude'] or -9999,
	city_code or 0,
	city_name or '',
	receiver_lon,
	receiver_lat,
	args['receiverDirection'] or '', 
	args['receiverSpeed'] or 0,
	args['receiverDistance'] or 0, 
	args['content'] or '',
	args['start_time'], 
	args['end_time'],
	args['level'],
	args['checkTokenCode'] or 0,
	args['tokenCode'] or '',
	args['callbackURL'] or '',
	args['start_time'], 
	args['bizid'],
...
function save_geography_message_info(args,cur_time,user)
	...
	if sender_lon~=0 and sender_lat~=0 then
		...
		local key = string.format("GEOtoMessage:%s:%s",grid_no,cur_time)
		...
		local key = string.format("GEOtoMessage:%s",cur_time)
	...
	return media_url,sender_lon,sender_lat,receiver_lon,receiver_lat,city_code,city_name 
end

-- 存储集团微博具体信息
function save_group_message_info(args,cur_time,media_url,sender_lon,sender_lat,receiver_lon,receiver_lat,city_code,city_name)

	--将集团微博具体信息存入redis中
	--KEY: bizid:sendInfo vaule:
	local message_redis_name = hash_function(args['bizid'])
	local key = string.format("sendInfo:%s",args['bizid'])
	args['messageType'] = args['messageType'] or 0
	
	-- 将各个信息格式化到一个table中
	local value  = message_info_key(args,media_url,sender_lon,sender_lat,receiver_lon,receiver_lat,city_code,city_name)

	value  = string.format("%s|%s|%s|%s|%s|%s|%s|%s|%s|%s",
	value,
	'',
	'',
	'',
	...
	'',
	args['groupID'] or '',
	args['receiveCrowd'] or '',
	args['receiveSelf']
	)
	only.log('D', 'message_redis_name = %s, value = %s', message_redis_name, scan.dump(value))
	
	-- key: sendInfo:a2c645043ce50a11e5bb40000c29ae7997
	-- value :
	-- "2064302565||1||http://v1.mmfile.daoke.me/dfsapi/v2/gainSound?isStorage=true%26group=dfsdb_8%26file=1457401032%3A480ca2aae4ce11e58faa984be10d2298.amr|lAmQoflGq1|0|0|0|0|-9999|0||0|0||0|0||1457427014|1457427614|20|0|||1457427014|a2c645043ce50a11e5bb40000c29ae7997|3|||||||000001111||0"

	local ok = redis_pool_api.cmd(message_redis_name,'SET',key,value)
	only.log('D', 'message_redis_name = %s', message_redis_name)
...
function group_save_message_to_msgredis(args)
	...
	--------------------------------------------------------------------------------------------------------------------------
	-- 1. 将发信息 senderAccountID 进行哈希,获得message_redis_name(返回的是不同的数据库) 
	...
	local senderAccountID = args['senderAccountID'] or  "unknow"	
	
	---------------------------------------------------------------------------------------------------------------------------
	--一个用户发送消息的记录
	--key信息为 [发送者用户ID:应用编号:当前发送时间]
	--可以这样理解,在key中要明确 哪个用户:什么设备:什么时候发送
	--KEY:sendmessage:[senderAccountID:appkey:time] value:time|bizid
	local key = string.format("sendMessage:%s:%s:%s", senderAccountID, args['appKey'], cur_time)
	
	only.log('D', 'key: %s, message_redis_name: %s', key, message_redis_name)
	-- 此处使用的redis数据库由上面的哈希函数计算,配置可以在deploy中查看
	-- key : sendMessage:lAmQoflGq1:2064302565:20160323100   member : 1458698515|a237eb4d62f09b11e5a260000c29ae7997
	...
	--------------------------------------------------------------------------------------------------------------------------
	--KEY:sendmessage:time value:senderAccountID:appkeyi
	--key = sendMessage:20160323103, value = lAmQoflGq1:2064302565
	local key = string.format("sendMessage:%s",cur_time)
	local value = string.format("%s:%s",senderAccountID,args['appKey'])
	only.log('D', 'key = %s, value = %s\n', key, value)
	...
	if args['groupID'] then
		...
		-- KEY:[accountid:appkey:groupID:time]:sendmessage value: time|URLi
		-- key	: voice:lAmQoflGq1:2064302565:111111111:20160323104, 
		-- value: 1458701359|http://v1.mmfile.daoke.me/dfsapi/v2/gainSound?isStorage=true%26group=dfsdb_8%26file=1457401032%3A480ca2aae4ce11e58faa984be10d2298.amr
		local key = string.format("voice:%s:%s:%s:%s",senderAccountID,args['appKey'],args['groupID'],cur_time)
		local value = string.format("%s|%s",os.time(),args['multimediaURL'])
		only.log('D', 'if args[groupId] : key = %s, value = %s')
		...
		-- key : voice:20160323105, value : lAmQoflGq1:2064302565:111111111
		local key = string.format("voice:%s",cur_time)
		local value = string.format("%s:%s:%s",senderAccountID,args['appKey'],args['groupID'])

		only.log('D', 'key = %s, value = %s', key, value)
		local ok = redis_pool_api.cmd(message_redis_name,'SADD',key,value) 
		...
		--数据类型为集合
		-- message_redis_name = message_redis2
		-- key = fileLocation:a227e6037cf0a411e5999d000c29ae7997
		-- value = lAmQoflGq1:2064302565:111111111
		local message_redis_name = hash_function(args['bizid'])
		...
		only.log('D', 'message_redis_name = %s, key = %s, value = %s', message_redis_name, key, value)
		...
		--存储地理信息
		--media_url = http://v1.mmfile.daoke.me/dfsapi/v2/gainSound?isStorage=true%26group=dfsdb_8%26file=1457401032%3A480ca2aae4ce11e58faa984be10d2298.amr,
		--sender_lon = 0, sender_lat = 0, receiver_lon = 0, receiver_lat = 0, city_code = nil, city_name = nil
		local media_url,sender_lon,sender_lat,receiver_lon,receiver_lat,city_code,city_name = save_geography_message_info(args,cur_time,"group")

		only.log('D', 'media_url = %s, sender_lon = %s,sender_lat = %s, receiver_lon = %s, receiver_lat = %s, city_code = %s, city_name = %s', media_url,sender_lon,sender_lat,receiver_lon,receiver_lat,city_code,city_name)
		-----------------------------------------------------------------------------------------------------------------
		--存储集团微博的具体信息
		--
		--media_url = http://v1.mmfile.daoke.me/dfsapi/v2/gainSound?isStorage=true%26group=dfsdb_8%26file=1457401032%3A480ca2aae4ce11e58faa984be10d2298.amr,
		--sender_lon = 0,sender_lat = 0, receiver_lon = 0, receiver_lat = 0, city_code = nil, city_name = nil
		save_group_message_info(args,cur_timei,media_url,sender_lon,sender_lat,receiver_lon,receiver_lat,city_code,city_name)
...
function save_personal_message_info(args,cur_time,media_url,sender_lon,sender_lat,receiver_lon,receiver_lat,city_code,city_name)
	...
	args['messageType'] = args['messageType'] or 1

	local value  = message_info_key(args,media_url,sender_lon,sender_lat,receiver_lon,receiver_lat,city_code,city_name)

	value  = string.format("%s|%s|%s|%s|%s|%s|%s|%s|%s|%s",
	value,
	args['senderType'] or 3,
	args['geometryType'] or 0,
	args['commentID'] or '',
	args['receiverAccountID'] or '', 
	args['adTalkId'] or '', 
	args['POITD'] or '',
	'',
	'',
...
function city_save_message_to_msgredis(args,region_name,count)

	local cur_time = os.date("%Y%m%d%H%M")
	local cur_time = math.floor(cur_time/10)
	local message_redis_name = hash_function(args['bizid'])
	--十分钟内发送的所有城市信息
	--KEY : cityMessage:time
	local key = string.format("cityMessage:%s",cur_time)
	local value = args['bizid']
	local ok = redis_pool_api.cmd(message_redis_name,'SADD',key,value)

	if not ok then 
		only.log('E',string.format(" **save city message to redis failed! key:%s ,value:%s **", key, value))
	end

	--将城市在线具体信息存入redis中
	--KEY: bizid:cityMessageInfo vaule:
	local key = string.format("cityMessageInfo:%s",args['bizid'])

	local value  = string.format("%s|%s|%s|%s|%s|%s|%s|%s",
	args['regionCode'] or '',
	region_name or '' ,
	args['groupID'] or '',
	args['senderAccountID'] or '',
