...
local function get_dfs_data_by_http(dfsurl)
    ...
    local tmp_domain = string.match(dfsurl,'https://[%w%.]+:?%d*/')
    if not tmp_domain then return false,'' end 
    local domain = string.match(tmp_domain,'https://([%w%.]+:?%d*)/')
    ...
    if host == '127.0.0.1' then
        ...
        if not f then 
            print(string.format("open file failed! %s", file_full_path))
            return false ,'','' 
        end
        local file_data = f:read("*all")
        f:close()

        if not check_is_amr(file_data,#file_data) then
            print(string.format("local voice file is invalid: %s", dfsurl) )
            return false, '', ''
        end
    ...
    if not ok then
        print('get dfs data when send get request failed!')
        return false,'','' 
    end
    ...
    if not check_is_amr(file_data,#file_data) then
        ...
        print(string.format("voice file is invalid: %s", dfsurl) )
        return false, '', ''
    end
    return true , filey_tpe, file_data
end
...
local function get_https_amr_file()
    local file_url = "https://api.weixin.qq.com/cgi-bin/media/get?access_token=AChMWTtIO7Fu5af_FAE2xSfsXKpqRX31nfN_b-iDFdcuQTpFjiPgcAkQqPor0VNmehGX08QH0FuXQARqhDXoui6ybZY6ot8oNXUfp5MtIAY&media_id=yfG3ME13DKYPNnDq3YH_wUa4QR_RUsBxFPHz5bCxOUlfTHT_v-fJdoLmcaNOIXyj"
    local ok, file_type, file_data = get_dfs_data_by_http(file_url)
    print(ok)
    print(file_type)
    if file_data then
        print(#file_data)
