...
local G = {
	...
	---- 检查重复申请
	sql_check_repeat_apply = "select 1 from checkSecretChannelInfo WHERE name = '%s' and introduction = '%s' and cityCode = %s and " ..
								"  catalogID = %s  and logoURL = '%s' and isVerify = %s and openType = %s and keyWords = '%s' ",
	...
	sql_finish_apply_channel = "insert into checkSecretChannelInfo(idx,number,name,introduction,cityCode,cityName,catalogID,catalogName," .. 
							" logoURL,openType,capacity,accountID,createTime,inviteUniqueCode,isVerify,keyWords,channelNameURL,userCount)" ..
	...
	---- 更新次数表
	sql_update_userAdminInfo = "update userAdminInfo set `create` = `create` + 1 ,updateTime = %d where accountID = '%s' ",
...
local function check_parameter(args)
	...
	if app_utils.str_count(channel_intro) < 1 or app_utils.str_count(channel_intro) > 128 
					 or (string.find(args['channelIntroduction'],"'")) then
		gosay.go_false(url_tab,msg['MSG_ERROR_REQ_ARG'],'channelIntroduction')
	end
	...
	--频道所属城市
	if not args['channelCityCode'] or not utils.is_number(args['channelCityCode']) or
				not (#args['channelCityCode'] == 6 or (tonumber(args['channelCityCode']) or -1 ) == 0 ) then
		gosay.go_false(url_tab,msg['MSG_ERROR_REQ_ARG'],'channelCityCode')
	end

	--频道类型
	if not args['channelCatalogID'] or not utils.is_number(args['channelCatalogID']) or 
			#tostring(args['channelCatalogID']) < 6 or #tostring(args['channelCatalogID'])  > 10 then
		gosay.go_false(url_tab,msg['MSG_ERROR_REQ_ARG'],'channelCatalogID')
	end

	--  频道是否开放
	local open_type = args['openType']
	if not open_type or not (open_type == "0" or open_type == "1")  then
...
local function check_repeat_apply( accountid, channel_name, channel_intro,  channel_citycode , 
											channel_catalogid ,
											open_type,
											channel_logo_url,
											is_verify,
	...
	---- 用户最多能申请50个频道
	---- 群聊频道用户申请数限制
	local sql_str = string.format(G.sql_check_channel_count,accountid)
	local ok , tab = mysql_api.cmd(channel_dbname,'select',sql_str)
	if not ok or not tab or #tab == 0 then
		only.log('E',"sql_check_channel_count failed   %s   ",sql_str) 
		gosay.go_false(url_tab,msg['MSG_DO_MYSQL_FAILED'])
	end

	if tonumber(tab[1]['count']) >= 10 then
		only.log('E',"cur channel is full [%s]",sql_str)
		gosay.go_false(url_tab,msg['MSG_ERROR_USER_CHANNEL_MAX_COUNT'])
	end

	---- 判断完全相同的
	local sql_str = string.format(G.sql_check_repeat_apply,channel_name,channel_intro,channel_citycode,
									channel_catalogid,channel_logo_url,is_verify,open_type,channel_keyWords)
	...
	if #tab >= 1 then
		only.log('E', "cur channel repeat apply [%s] ", sql_str ) 
...
--去掉频道编号 自动生成
local function apply_user_secret_channel( accountid, channel_name, 
														channel_intro, 
														channel_citycode, 
														channel_catalogid ,  
														open_type , 
	...
	check_repeat_apply( accountid, channel_name, channel_intro,  channel_citycode , channel_catalogid ,open_type,channel_logo_url,is_verify,channel_keyWords)

	local cur_time = ngx.time()

	local tmp_channelname = string.format("群聊频道%s" ,channel_name)
	...
	if not ok_status or not ok_channelname_url or #ok_channelname_url < 1 then
		only.log('W',string.format(' channel_name:%s  txt2voice failed !' , tmp_channelname  ) )
	end
	
	local ok,channel_cityname = nil
	if #channel_citycode == 6 then
		ok,channel_cityname = appfun.get_city_name_by_city_code ( channel_citycode )
		if not ok or not channel_cityname then
			only.log('E', "get_city_name_by_city_code failed %s : %s ", accountid, channel_citycode)  
			gosay.go_false(url_tab, msg['MSG_ERROR_REQ_ARG'],"channelCityCode")
		end
	elseif #channel_citycode == 1 then
		only.log('D',"***channel_citycode:%s",channel_citycode)
		channel_cityname = '全国'
	else
		only.log('E',"channelCityCode is %s",channel_citycode)
		gosay.go_false(url_tab,msg['MSG_ERROR_REQ_ARG'],'channelCityCode')
	end
	...
	if #ret == 0 then
		...
		sql_str = string.format(G.sql_update_userAdminInfo,cur_time,accountid)
	end
	...
	local sql_str = string.format(G.sql_finish_apply_channel, channel_idx , channel_num, 
															...
															channel_keyWords,ok_channelname_url)

	table.insert(sql_tab,sql_str)
	local sql_str = string.format(G.sql_insert_checkSecretChannelInfoHistory,cur_time,appkey,accountid,channel_citycode,channel_catalogid,is_verify)
	...
	return true,channel_num,uniquecode
end
...
local function  handle()
	...
	local ok, channel_num, uniquecode = apply_user_secret_channel( accountid,  
							channel_name, 
							channel_intro, 
							channel_citycode, 
							channel_catalog , 
							...
							appkey,
							channel_keyWords)
	if ok and channel_num and uniquecode then
		----SUCCED
		local ret = string.format('{"channelNumber":"%s","uniqueCode":"%s"}',channel_num,uniquecode)
