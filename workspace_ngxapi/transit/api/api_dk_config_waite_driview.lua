...
local transmit_data = require('transmit_data')
-- ---- 终端 IMEI 类型标志 ------------------------------
...
local G = {
	...
	sql_check_mod  = "select 1 as F from modelInfo   where validity = 1 and model = '%s' and  %s <= endTime limit 1 ",
	sql_config_info =  " select  accountID,call1,call2,domain,port,customArgs from configInfo where accountID = '%s' limit 1 " ..
					" union all select  accountID,call1,call2,domain,port,customArgs from configInfo where accountID = '' and model='%s' limit 1 ",
	sql_user_poweron = "INSERT INTO userPowerOnInfo(createTime, accountID, imei,imsi,model, tokenCode, tokenCodeEndTime,travelID,version) values(%s, '%s', %s, %s, '%s' , '%s' , %s ,%s , '%s' )",
}
...
local function save_poweron_to_table( imei,imsi,mod,token_code ,version )
	...
	local sql_str = string.format(G.sql_user_poweron,G.cur_time  ,(G.accountid or ''), imei,imsi,mod,token_code, (G.cur_time + 48*60*60) ,travel_id,version)
...
local function update_channel_info_to_redis(accountid, voiceCommand )
	...
	if #accountid == 15 then
		...
		---- 仅仅兼容老版本业务数据 2015-06-26
		if not ok_channel then
			ok_channel = default_channel
		end
	...
	---- 设置用户关注的微频道
	local ok, ret = redis_api.cmd('private','smembers', string.format("%s:userFollowMicroChannel",accountid))
...
local function ready_execution(imei,imsi,mod,version)
	local result_str = '{"call1":"%s","call2":"%s","domain":"%s","port":%s,"tokencode":"%s","customargs":%s}'
	----获取accountID
	...
	if G.cur_type == dk_utils.DK_IMEI_MANUFACTURE then
		update_channel_info_to_redis(tmp_accid, custom_tab['voiceCommand'] )
	end
	---------------------------------------------------------------------------------------------------
	...
	if G.cur_type == dk_utils.DK_IMEI_ENGINEERING then
		cur_doman =  "s9gc.mirrtalk.com"  --"s9gc.mirrtalk.com"
		cur_port = "80"
	end

	--domain,port,customArgs
	local result_txt = string.format(result_str, ok_config[1]['call1'], ok_config[1]['call2'] ,
									cur_doman,
									cur_port,
									token_code,
									custom_str)
...
local function post_poweron_status_to_data_routing(args)
	local body_data = string.format('{"powerOn":true,"accountID":"%s","tokenCode":"%s","model":"%s","IMEI":"%s"}',
					...
					args['mod'],
					args['imei'])
	local _,ok_status = pcall(transmit_data.post_data_to_power_info,args['imei'],body_data)
        if not ok_status then
                only.log('W',"transmit data to driview failed:%s",body_data)
                return false
        end
	return true
end
