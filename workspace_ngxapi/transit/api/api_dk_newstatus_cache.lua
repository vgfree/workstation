...
local function fetch_weibo_pre(user, user_type)
	...
	repeat
		...
		-- eval "local tab = redis.call('zrange','%s',0,0)  if #tab > 0 then redis.call('zrem','%s',tab[1])  return tab[1] end  return nil "
...
local function handle()
	...
	-- if not ok then
	-- 	only.log('E',string.format('get imei accountid failed!', G.imei ))
	-- 	go_exit()
	-- end
