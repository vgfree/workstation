...
local function weibo_expired(weibo)
...
local function fetch_weibo(user, user_type)
	...
	repeat
		...
		---- 2015-01-12 
		local ok, bizid = redis_api.cmd("weibo","eval", string.format(G.redis_eval_command, user, user  ) ,0)
		if not ok or not bizid then
			return nil,nil
		else
			-------- 先删除微博队列
			------ redis_api.cmd("weibo", "zrem", get_weibo_key, bizid)
			local ok,raw_weibo = redis_api.cmd("weibo", "get", bizid .. ":weibo")
			if not ok then
			...
			elseif ok and raw_weibo then
				...
				if begin_bizid == 'a3' or begin_bizid == 'a4' then
					---- 删除个人微博
					redis_api.cmd("weibo", "del", bizid .. ":weibo")
				end
				...
				if not ok then
					...
					---- 判断微博有效期
					if weibo_expired(weibo_info)  then
						redis_api.cmd("weibo", "del", bizid .. ":weibo")
						redis_api.cmd('weibo', 'del', bizid .. ':senderInfo')
					elseif   ok_groupMsg > cur_groupMsgTime and ok_groupMsg > 0  and (tonumber(weibo_info['level']) or 0)  > 21 then
						...
						---- 统计微博读取信息情况
						---- 
						local ok,ret = redis_api.cmd('weibo','get', bizid .. ':senderInfo')
						if ok and ret  then 
...
local function handle()
	...
	---- 获取微博
	weibo, weibo_type = fetch_weibo( G.cur_accountid , 'weiboPriority')
	if not weibo then
	...
	if weibo['direction'] and #weibo['direction'] > 0  then
		result_tab['direction'] = weibo['direction']
	end

	if weibo['speed'] and #weibo['speed'] > 0 then
