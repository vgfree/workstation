...
local transit_cfg = require('transit_cfg')
...
local cur_local_path = transit_cfg.OWN_INFO.current_local_path
local cur_host_port  = transit_cfg.OWN_INFO.current_host_port
local cur_host_ip    = transit_cfg.OWN_INFO.current_host_ip
...
--	org_filesize = "",
	...
	sql_save_urllog = "insert into urlLog_%s(createTime,url,clientip,returnValue) values( %d , '%s' , inet_aton('%s') , '%s' )",
...
local function save_file_to_disk()
	if #G.file_binary > 1 and G.dfs_url == '' then
		...
		local f = io.open(string.format("%s/erramr/%s", cur_local_path, tmp_file_name), 'w')
		if f then
			f:write(G.file_binary)
			f:close()
			local file_name = string.format("http://%s:%s/erramr/%s", cur_host_ip, cur_host_port, tmp_file_name)
			return file_name
		else
...
local function get_last_timestamp(accountid)
	local ok_status,ok_timestamp  = redis_api.cmd("readgpsdata",'get',accountid .. ':currentGPSTime')
...
local function get_last_point( accountid)
	-- local ok_status,ok_tab  = redis_api.cmd('private','mget',accountid .. ':longitude', accountid .. ':latitude')
	-- if not ok_status or ok_tab == nil or #ok_tab ~= 2 then
	-- 	return 0,0
	-- end
	-- -- only.log('D',string.format("longitude:%s   latitude:%s " ,ok_tab[1] ,ok_tab[2] ))
	-- return (ok_tab[1] or 0 ) ,(ok_tab[2] or 0)

	local ok,gps_str = redis_api.cmd("readgpsdata",'get',accountid .. ":currentBL")
...
local function upload_file( file_name, file_binary  )
	...
	only.log('D',"xxxxxxxx in upload_file\n%s",req)
...
local function save_files(file_tab )
	...
	local ok_status, ok_url, ok_id , ok_size = upload_file( G.file_name ,  G.file_binary)
	if ok_status and ok_url then
		only.log('I',"[SUCCED] dfs upload_file succed!--->------%s",ok_url)
	else
...
local function save_file_to_dfs(args)
	...
	local v = args['mmfile']
	if not v then
		only.log('D','file no include mmfile')
		return false
	end
	
	if type(v) == "table" and  v['file_name'] and v['data'] then
			...
			ok_status ,G.dfs_url,G.file_id, G.file_size = save_files(v)
			if ok_status == true then
				--dfs调用成功
				v['data'] = G.dfs_url
			elseif v['data'] and ok_status == false then
				v['data'] = '[FAILED] [save file to dfs] '
				only.log('D','v[data]:%s',v['data'])
			end
	else
...
local function save_urllog(return_value)
	...
	if tonumber(dk_utils.DK_WRITE_MYSQL_WITH_URL) ~= 0 then
		local sql_str = string.format(G.sql_save_urllog,G.cur_month, G.cur_time, G.body,G.req_ip,return_value)
...
local function get_token_code(imei)
	local ok_status,token_code = redis_api.cmd('saveTokencode','get',imei .. ':tokenCode')
	if not ok_status then
...
local function check_parameter(args)
	...
	if not tmp or #tmp ~= 5 or string.upper(tmp) ~= tmp then
		only.log('D','[CHECK_FAILED]mod	#mod~=5 or string.upper(mod)~=mod')
		return false 
	end
	...
	if tmp_token_code ~= token_code then
		only.log('E','[CHECK_FAILED]tokencode invalid really tmp_tokencode is:%s----token_code is:%s ',tmp_token_code,token_code)
		return false
	end
	return true
end
...
--	end
	...
	---- http异步调用
	only.log('D',"[DEBUG] xxxx request feedbackDispatch  %s ",req )
...
local function save_feedback_info(args)
	...
	local sql_str = string.format(G.sql_save_feedback, 
								G.cur_month,
								G.accountid or '',
								args['imei'],args['imsi'],args['type'], G.cur_time, G.collect_time,G.GMTTime,
	...
	if not ok_status then
		only.log('E','save feedback info to database failed!  %s ',sql_str) 
		return false
	end
	return true
end

--将feedbackinfo 的具体信息，存储到redis里
--fix hou.y.q 2015-09-05
--key:feedbackInfo:time(10分钟)

local function save_feedback_info_to_message_redis(args)
	
	local cur_time = os.date("%Y%m%d%H%M")
	local cur_time = math.floor(cur_time/10)
	local accountID  = G.accountid
	
	if #tostring(G.accountid) == 0 then 
		accountID = args['imei']
	end

	local redis_key = string.format("feedbackInfo:%s:%s", accountID,cur_time)

	local tab_redis_value = {		[1] 	= accountID 					or '',
						[2] 	= args['imei'] 					or '',
						[3]	= args['imsi']					or '',	
						[4]	= args['type']					or 0 , 
						[5]	= G.cur_time					or 0 , 
						[6] 	= G.collect_time				or 0 ,
						[7] 	= G.GMTTime					or '',
						[8] 	= G.latitude * dk_utils.gps_point_unit,
						[9]	= G.longitude * dk_utils.gps_point_unit,
						[10]	= G.direction 					or 0 , 
						[11] 	= G.speed					or 0 , 
						[12] 	= G.altitude					or 0 ,
						[13]	= tonumber(args['mmfilemins']) 			or 0 , 
						[14] 	= dk_utils.DEFAULT_FILE_CHANNEL,
						[15]	= tonumber(args['mmfilekbps']) 			or 0 , 
						[16] 	= dk_utils.DEFAULE_FILE_BIT,
						[17]	= args['mmfiletype'] 				or '',
						[18] 	= G.file_id					or '',
						[19] 	= G.file_size 					or 0 , 
						[20] 	= G.dfs_url 					or '', 
						[21] 	= G.receive_time				or 0 ,
						[22]	= args['bizid'] 				or '', 
						[23] 	= args['tokencode']				or '',}
	
	
	local redis_value = table.concat(tab_redis_value,"|")
	
	local ok = redis_api.hash_cmd("message_hash",accountID,"SADD",redis_key,redis_value)
	if not ok then
		only.log('E',string.format(" **save feedback info to redis failed! key:%s ,value:%s **", redis_key, redis_value))
	end

	local redis_key = string.format("feedbackInfo:%s",cur_time)
	local ok= redis_api.hash_cmd("message_hash",accountID,'SADD',redis_key,accountID)

	if not ok then
		only.log('E',string.format(" **save feedback info to redis failed! key:%s ,value:%s **", redis_key, accountID))
	end
...
local function all_type_check(args)
	...
	if op_type == dk_utils.DK_TYPE_REPLYVOICE then --> 10 回复语音
	...
	elseif op_type == dk_utils.DK_TYPE_VOICE or op_type == dk_utils.DK_TYPE_COMMAND or op_type == dk_utils.DK_TYPE_GROUP or op_type == dk_utils.DK_TYPE_REPLYVOICE then
				-->3 发送语音			-->4 语音命令				-->5 发送群组语音
		only.log('E','xxxxxxxxxxxxxactionType is %s,',op_type)
		is_succed = check_fileinfo(args) --> 判断文件相关的参数是否正确
		if is_succed == true then
			is_succed = save_file_to_dfs(args)-->保存文件至dfs
			if is_succed ~= true then
				only.log('E','***Error*** actionType is %s, save file to dfs failed',op_type)
			end
		else
			only.log('E','***Error*** actionType is %s, check file info failed ',op_type)
		end
	...
	elseif op_type == dk_utils.DK_TYPE_POWEROFF or op_type == dk_utils.DK_TYPE_NOT_COMMAND or op_type == dk_utils.DK_TYPE_NOT_GROUP then
		is_succed = true
	end
	return is_succed
end
...
local function handle()
	...
	G.req_ip               = req_ip
	G.cur_time             = os.time() ----time 1403933518
	G.cur_month	       = string.gsub(string.sub(ngx.get_today(),1,7),"%-","")

	only.log('D','xxxxx11111')
	...
	G.imei         = args['imei']
	local ok_status,acc_id = redis_api.cmd(read_private,'get', G.imei .. ':accountID')
	if not ok_status then
	...
	G.receive_time = string.format("%d", G.cur_time )
	...
	--固化数据存储修改
	--hou.y.q 2015-09-05
	--Mysql数据量太大，将2小时数据先存储到redis里，再固化到tsdb里
	save_feedback_info_to_message_redis(args)
	---- 所有成功,将调用该接口转发数据至api
	if is_succed == true then
