...
local function get_last_timestamp(accountid)
	local ok_status,ok_timestamp  = redis_api.cmd(read_private,'get',accountid .. ':currentGPSTime')
...
local function get_last_point( accountid)
	...
	local ok,gps_str = redis_api.cmd(read_private,'get',accountid .. ":currentBL")
...
local function upload_file( file_name, file_binary  )
	...
	----- 2014-07-02
	----- jiang z.s. 
	----- amrToType = "1"  警告,暂时添加该参数,上传的amr文件,同时也会转换为mp3文件存储给微信(web)端使用
...
local function get_token_code(imei)
	local ok_status,token_code = redis_api.cmd(read_private,'get',imei .. ':tokenCode')
	if not ok_status then
...
local function all_type_check(args)
	...
	if op_type == dk_utils.DK_TYPE_CALL1 or op_type == dk_utils.DK_TYPE_CALL2 then
		-->1  call1                    	-->2  call2 
		is_succed = true
	elseif op_type == dk_utils.DK_TYPE_VOICE or op_type == dk_utils.DK_TYPE_COMMAND or op_type == dk_utils.DK_TYPE_GROUP then
