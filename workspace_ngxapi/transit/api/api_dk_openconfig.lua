...
local function handle()
	...
	only.log('D', "appkey=%s",args['appKey'])
        only.log('D', "sign = %s", args['sign'])
        only.log('D', "text = %s", args['text'])
--------------------------------------------------------
        local location = "spx_txt_to_voice";
        local server = {
                        host = '192.168.71.84',
                        port = 1111,
                 }

        local req_type = "application/x-www-form-urlencoded"
        local req_data = utils.post_data(location, server , req_body, req_type )
        only.log('D', "req_data=%s",req_data)
        ret_data = http_api.http( server, req_data, true , 1500)
        only.log('D', "ret_data=%s",ret_data)
--------------------------------------------------------
