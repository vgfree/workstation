...
local function weibo_expired(weibo_endtime)
...
---- 保存已经读取的bizid微博 
local function save_read_bizid( accountid, args, read_bizid )
	-- local sql_tab = {
...
local function fetch_weibo(user, user_type)
	...
	repeat
		...
		---- 2015-01-12 
		local ok, bizid = redis_api.cmd(weibo_redis ,"eval", string.format(G.redis_eval_command, user, user  ) ,0)
		if not ok or not bizid then
			return nil,nil
		else
			-------- 先删除微博队列
			------ redis_api.cmd("weibo", "zrem", get_weibo_key, bizid)
			local ok, weibo_info = redis_api.cmd(weibo_redis, "hgetall", bizid .. ":weibo")
			if not ok then
			...
			elseif ok and weibo_info then
				...
				if begin_bizid == 'a3' or begin_bizid == 'a4' then
					---- 删除个人微博
					redis_api.cmd(weibo_redis, "del", bizid .. ":weibo")
				end
				weibo_info['bizid'] = bizid

				---- 判断微博有效期
				if weibo_expired(weibo_info['endTime'])  then
					redis_api.cmd(weibo_redis, "del", bizid .. ":weibo")
					redis_api.cmd(weibo_redis, 'del', bizid .. ':senderInfo')
				elseif   ok_groupMsg > cur_groupMsgTime and ok_groupMsg > 0  and (tonumber(weibo_info['level']) or 0)  > 21 then
					...
					---- 统计微博读取信息情况
					local ok,tmp_tab = redis_api.cmd(weibo_redis,'hgetall', bizid .. ':senderInfo')
...
local function handle()
	...
	if args["bizid"] and #args["bizid"]  > 0 then
		...
		save_read_bizid( accountid, args, args["bizid"] )
	...
	---- 获取微博
	weibo = fetch_weibo( G.cur_accountid , 'weiboPriority')
	if not weibo then
		only.log('D', " %s weibo is nil---->----return 555 " , G.cur_accountid ) 
		go_empty()
	end
	...
	local dfsurl = weibo['mediaFileURL'] or ''
	...
	---- 下发音频类型
	local w_type = tonumber(weibo['messageType'])
	---- 微频道增加前缀
