...
local function fetch_weibo_pre(user, user_type)
	...
	repeat
		local ok, bizid = redis_api.cmd(weibo_redis_pre,"eval", string.format(G.redis_eval_command, user, user  ) ,0)
		if not ok or not bizid then
			return nil,nil
		else
			---- 先删除微博队列
			-- redis_api.cmd("weibo", "zrem", get_weibo_key, bizid)
			local ok,raw_weibo = redis_api.cmd(weibo_redis_pre, "get", bizid .. ":weibo")
			if not ok then
				only.log('E',  " get %s:weibo info failed %s" ,bizid , raw_weibo) 
...
local function get_adtalk_info( cur_longitude, cur_latitude, cur_appkey  )
	...
	only.log('W',"********cur_longitude:%s,cur_latitude:%s************** ", cur_longitude, cur_latitude) 
	...
	only.log('W',"adtalk redis-key %s " , cur_adtalk_key ) 
	...
	if ok and ret and type(ret) == "table" and #ret == 1 then
		...
		if ok and info then
			...
			only.log('W',"adtalk_info get status:%s adtalk_id:%s adtalk_url:%s " , cur_append_status ,adtalk_id , cur_append_url )
