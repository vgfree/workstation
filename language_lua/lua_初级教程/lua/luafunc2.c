...
int getstring(lua_State *L)
{
	...
	char * name=luaL_checkstring(L,1);//获取lua第一个参数
	char buf[100];
	memset(buf,0,100);
	sprintf(buf,"NNNN%s",name);
...
int main (int argc, char **argv) {
	...
	luaL_openlibs(L);// 把所有标准类库加载到指定的虚拟机
//	dofile(L,"test.lua");
//
	lua_register(L,"getstring",getstring);
	printf("return = %d\n",luaL_loadfile(L,"test3.lua")); // 加载lua脚本
	lua_call(L,0,1);// 调用Lua脚本
	lua_close(L);
	return 0;
